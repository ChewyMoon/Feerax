...
namespace Feerax.Engine.Maps
{
    class Xareef : Map
    {
        ...
        public override IEnumerable<Block> GetBlocks()
        {
            ...
            // Add bottom platform
            for (var i = 0; i < Feerax.Instance.GraphicsDevice.Viewport.Width / (Grass.GrassTexture.Width * Grass.Scale); i++)
            {
                _blocks.Add(new Grass {Position = new Vector2(i * Grass.GrassTexture.Width * Grass.Scale, Grass.GrassTexture.GraphicsDevice.Viewport.Height - (Grass.GrassTexture.Height * Grass.Scale)) });
            }
